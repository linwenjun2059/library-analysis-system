...
-- 16. 资源优化建议表（高级管理员）
DROP TABLE IF EXISTS `resource_optimization`;
CREATE TABLE `resource_optimization` (
  ...
  PRIMARY KEY (`optimization_type`, `book_id`),
  KEY `idx_book_id` (`book_id`),
  KEY `idx_priority` (`priority`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='资源优化建议表';
...
-- 19. 馆藏流通率表（高级管理员）
DROP TABLE IF EXISTS `circulation_rate`;
CREATE TABLE `circulation_rate` (
  ...
  `circulation_rate` DOUBLE DEFAULT NULL COMMENT '流通率',
  `avg_borrow_times` DOUBLE DEFAULT NULL COMMENT '平均借阅次数',
  `high_demand_books` BIGINT(20) DEFAULT NULL COMMENT '高需求图书数',
  `zero_borrow_books` BIGINT(20) DEFAULT NULL COMMENT '零借阅图书数',
  `turnover_rate` DOUBLE DEFAULT NULL COMMENT '周转率',
  PRIMARY KEY (`dimension_type`, `dimension_value`(191)),
  KEY `idx_circulation_rate` (`circulation_rate`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='馆藏流通率分析表';
...
14.1 recommendation_details - 推荐策略明细表（多算法对比）
14.2 recommendation_stats - 推荐效果统计表（推荐系统监控）
15. user_profile - 用户画像分析（高级管理员）
16. resource_optimization - 资源优化建议（高级管理员）
17. major_reading_profile - 专业阅读特征（高级管理员）
18. overdue_analysis - 逾期分析（图书管理员）
19. circulation_rate - 馆藏流通率（高级管理员）
