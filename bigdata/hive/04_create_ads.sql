...
-- 馆藏利用分析（高级管理员）- 删除无价值的“流通率”，保留周转率等指标
DROP TABLE IF EXISTS ads_collection_utilization;

CREATE TABLE IF NOT EXISTS ads_collection_utilization(
    dimension_type STRING COMMENT '维度类型（位置/主题）',
    dimension_value STRING COMMENT '维度值（具体位置或主题名称）',
    total_books BIGINT COMMENT '馆藏图书总数',
    borrowed_books BIGINT COMMENT '被借图书数',
    total_lend_count BIGINT COMMENT '总借阅次数',
    avg_borrow_times DOUBLE COMMENT '平均借阅次数（次/本）',
    avg_borrow_days DOUBLE COMMENT '平均借阅天数',
    turnover_rate DOUBLE COMMENT '周转率（次/本/年）= 总借阅次数 / 馆藏总数 / 时间跨度(2年)',
    high_demand_books BIGINT COMMENT '高需求图书数（借阅>5次）',
    medium_demand_books BIGINT COMMENT '中等需求图书数（2-5次）',
    low_demand_books BIGINT COMMENT '低需求图书数（1次）',
    unique_readers BIGINT COMMENT '独立读者数',
    reader_per_book_ratio DOUBLE COMMENT '读者/图书比 = 独立读者数 / 馆藏总数'
)
COMMENT '馆藏利用分析表（位置/主题维度，含周转率、需求分级、读者比）'
STORED AS PARQUET
TBLPROPERTIES (
    'parquet.compression'='SNAPPY'
);
...
-- 图书关联规则表（FPGrowth算法）
DROP TABLE IF EXISTS ads_book_association;

CREATE TABLE IF NOT EXISTS ads_book_association(
    antecedent ARRAY<STRING> COMMENT '前项图书ID列表',
    consequent ARRAY<STRING> COMMENT '后项图书ID列表',
    antecedent_title STRING COMMENT '前项图书标题',
    consequent_title STRING COMMENT '后项图书标题',
    antecedent_subject STRING COMMENT '前项图书主题',
    consequent_subject STRING COMMENT '后项图书主题',
    confidence DOUBLE COMMENT '置信度（0-1）',
    lift DOUBLE COMMENT '提升度（>1表示正相关）',
    support DOUBLE COMMENT '支持度',
    rule_description STRING COMMENT '规则描述',
    association_type STRING COMMENT '关联类型（同主题关联/跨主题关联）'
)
COMMENT '图书关联规则表（FPGrowth算法挖掘结果）'
STORED AS PARQUET
TBLPROPERTIES (
    'parquet.compression'='SNAPPY'
);
...
CREATE TABLE IF NOT EXISTS ads_user_cluster(
    userid STRING COMMENT '用户ID',
    dept STRING COMMENT '院系',
    user_type STRING COMMENT '用户类型',
    occupation STRING COMMENT '专业',
    cluster INT COMMENT '聚类编号',
    cluster_name STRING COMMENT '聚类名称（如：考研学习族）',
    cluster_characteristics STRING COMMENT '聚类特征描述',
    borrow_count BIGINT COMMENT '借阅量',
    active_days INT COMMENT '活跃天数',
    overdue_rate DOUBLE COMMENT '逾期率',
    avg_borrow_days DOUBLE COMMENT '平均借阅天数',
    reading_breadth INT COMMENT '阅读广度（涉及主题数）'
)
COMMENT '用户聚类分群表（K-means算法结果）'
STORED AS PARQUET
TBLPROPERTIES (
    'parquet.compression'='SNAPPY'
);
