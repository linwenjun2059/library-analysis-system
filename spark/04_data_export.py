...
"""
步骤4：数据导出（Hive → MySQL）
统一导出21张表到MySQL供业务系统使用

表分类：
- 维度表（3张）：用户维度、图书维度、近期借阅记录
- 汇总表（5张）：用户汇总、图书汇总、院系汇总、主题汇总、每日统计
- 聚合表（5张）：热门图书、活跃用户、院系偏好、借阅趋势、运营看板
- 功能表（8张）：用户画像、资源优化、专业阅读、逾期分析、流通率、时间分布、用户排名、推荐基础表

注：推荐表（2张）由05_book_recommend.py单独导出
"""
...
class DataExporter:
    ...
    def _export_user_dimension(self):
        """导出用户维度表"""
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出用户维度表...")
    ...
    def _export_book_dimension(self):
        """导出图书维度表"""
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出图书维度表...")
    ...
    def _export_recent_lend_records(self):
        """导出近期借阅记录（数据中最近6个月）"""
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出近期借阅记录（数据中最近6个月）...")
    ...
    def export_summary_tables(self):
        ...
        # 每日统计（数据中最近1年）
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出每日统计（数据中最近1年）...")
    ...
    def export_feature_tables(self):
        """导出功能表（8张）- 支持高级管理员、图书管理员、普通用户功能"""
        print("\n" + "█" * 60)
        print("第四部分：导出功能表（8张）")
        print("█" * 60)
        
        self._export_user_profile()
        self._export_table("resource_optimization", "library_ads.ads_resource_optimization", "资源优化建议")
        self._export_major_reading_profile()
        self._export_table("overdue_analysis", "library_ads.ads_overdue_analysis", "逾期分析")
        self._export_table("circulation_rate", "library_ads.ads_circulation_rate", "馆藏流通率")
    ...
    def _export_user_profile(self):
        ...
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出用户画像分析...")
    ...
    def _export_major_reading_profile(self):
        ...
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出专业阅读特征...")
    ...
    def _export_table(self, mysql_table, hive_table, desc):
        """通用导出方法"""
        print("\n" + "=" * 60)
        print(f"[{self.exported_count + 1}/21] 导出{desc}...")
    ...
    def run(self):
        ...
        print("   - 功能表：8张（用户画像/资源优化/专业阅读/逾期分析/流通率/时间分布/用户排名/推荐基础）")
        print("💡 推荐表：2张由05_book_recommend.py单独导出")
        print("█" * 60)
