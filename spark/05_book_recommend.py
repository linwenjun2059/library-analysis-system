...
class BookRecommender:
    ...
    def collaborative_filtering(self):
        ...
        # 用户和图书ID转换为数值型（ALS要求Integer类型）
        # 优化：使用zipWithIndex()替代窗口函数，避免全局排序性能问题
    ...
    def content_based_recommend(self):
        ...
        # 合并用户偏好
        user_preferences = user_subject_pref \
            .join(user_author_pref, "userid", "left") \
            .join(user_borrowed_books, "userid")
        ...
        content_recommendations = content_recs \
            ...
            .withColumn(
                "subject_match",
                when(array_contains(col("preferred_subjects"), col("subject")), 1.0).otherwise(0.0)
            ) \
            .withColumn(
                "author_match",
                when(array_contains(col("preferred_authors"), col("author")), 1.0).otherwise(0.0)
            ) \
            .withColumn(
                "raw_score",
            ...
            .withColumn(
                "content_score",
                round(col("raw_score") * 10, 2)  # 归一化：0.4-1.0 -> 4-10分，保疙2位
            ) \
            .withColumn(
                "rank",
    ...
    def save_recommendations(self, recommendations, cf_recs, content_recs, popularity_recs):
        ...
        user_diversity = recommendations.groupBy("userid").agg(
            ...
            "diversity_score",
            (col("subject_count") / col("total_count") + col("author_count") / col("total_count")) / 2
        ...
        recommend_struct = recommendations.groupBy("userid") \
            ...
            .withColumn("recommend_date", current_date().cast("string")) \
